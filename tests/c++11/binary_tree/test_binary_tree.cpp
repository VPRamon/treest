...
TEST_F(BinaryTreeTest, TestNodeProperties) {
...
TEST_F(BinaryTreeTest, TestGetChildren) {
    auto& child1 = tree.getNode(1);
...
TEST_F(BinaryTreeTest, TestExceptionSafety) {
    EXPECT_THROW(tree.getNode(-1), std::out_of_range);
    EXPECT_THROW(tree.getNode(100), std::out_of_range);
...
TEST_F(BinaryTreeTest, TestOperatorOverloading) {
    std::stringstream ss;
    ss << tree;
...
/*
TEST_F(BinaryTreeTest, TestVariantNode) {
...
*/
/*
TEST_F(BinaryTreeTest, TestShouldNotCompile) {
    auto root = tree.getRoot();
